...
CONST C_APPVERSION='1.1.20120328';
CONST C_DEFAULTXMLFILE='coursedata.xml';
type
  ...
  lbl_BogeyRating.Caption:=Format('%.1f',[HCPGolfUMLClass.BogeyRating]);
     if HCPGolfUMLClass.Metric then
        begin
             ...
             lbl_TotalTeeDistance.Caption:=Format('%dm',[HCPGolfUMLClass.TotalTeeDistance]);
        end
     else
         begin
              lbl_InTeeDistance.Caption:=Format('%dyds',[HCPGolfUMLClass.InTeeDistance]);
              lbl_OutTeeDistance.Caption:=Format('%dyds',[HCPGolfUMLClass.OutTeeDistance]);
              lbl_TotalTeeDistance.Caption:=Format('%dyds',[HCPGolfUMLClass.TotalTeeDistance]);
         end;
...
//  HCPGolfUMLClass:=TGolfmlClass.Create;
  ...
  If NOT HCPGolfUMLClass.InitOK then HALT;
  ...
  if bcoursedata then HCPGolfUMLClass.GetCourseInfoFromFile;

  cmb_courseList.Items.Clear;
  if bcoursedata then cmb_courseList.Items:=HCPGolfUMLClass.CourseStringList;
  ...
  if bcoursedata then RefreshDisplay
  else MessageDlg('Use File menu/Import Golfml file to load GolfML data',mtInformation,[MBOK],0);
end;
...
begin
     If opt_units.ItemIndex=0 then HCPGolfUMLClass.Metric:=TRUE
     else HCPGolfUMLClass.Metric:=FALSE;
     RefreshDisplay;
end;
...
begin
     ...
     If Length(s)=Length(HCPGolfUMLClass.ClubName) + 1 then s:='Sorry, no further information';
